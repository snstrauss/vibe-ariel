...
import { MermaidGraph, MermaidNode, MermaidEdge, MermaidSubgraph } from './types.js';
...
export class MermaidProcessor {
  ...
  private extractGraph(element: any): MermaidGraph {
    ...
    if (element.type && typeof element.type === 'function' && element.type.name === 'Graph') {
      // This is a Preact element created by JSX, extract the props
      const { type = 'flowchart', direction = 'TB', title, children, ...rest } = element.props;
  ...
  private processChildren(children: any): { nodes: MermaidNode[], edges: MermaidEdge[], subgraphs: MermaidSubgraph[] } {
    ...
    const processElement = (element: any) => {
      ...
      if (element && typeof element === 'object' && !element.type && Array.isArray(element)) {
        element.forEach(processElement);
        return;
      }
          ...
          if (element.type && typeof element.type === 'function') {
            ...
            switch (componentName) {
              ...
              case 'ThickArrow':
                ...
                break;
              case 'Subgraph':
                subgraphs.push(this.processSubgraphElement({
              ...
              default:
                ...
                if (componentName && componentName !== 'Graph') {
                  try {
                    ...
                    console.warn(`Failed to process custom component ${componentName}:`, error);
                  }
  ...
  private getNodeType(componentName: string): string {
    switch (componentName) {
      ...
      case 'DoubleCircle': return 'double_circle';
  ...
  private getEdgeType(componentName: string): string {
    switch (componentName) {
      ...
      case 'DottedArrow': return 'dotted';
      case 'DashedArrow': return 'dashed';
      case 'ThickArrow': return 'thick';
...
export function createMermaidElement(type: string, props: any, ...children: any[]) {
  if (type.startsWith('mermaid:')) {
    return createElement(type, { ...props, children: children.length === 1 ? children[0] : children });
  }
